...
export class Movies extends Component {
    ...
    render() {
        ...
        const options = movies.map((movie,i) => <option key={i} value={movie.id}>{movie.title}</option>)
        return (
            <div>
                ...
                {failedApiCall? <h1>We've hit a snag! Refresh and try again or come back later!</h1> : null}
            </div>
        )
    }
}
